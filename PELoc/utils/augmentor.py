...
class AugmentParams(object):
    ...
    def __init__(self, p_flipx=0.5, p_flipy=0.5,
...
class Augmentor(object):
    ...
    def SSDA(self, pointcloud):
        # # flip augment
        # print(self.parmas.p_flipx)
        rand = random.uniform(0, 1)
        # # print(rand)
        if rand < self.parmas.p_flipx:
            # print("-----flip-----xy")
            pointcloud = self.flipX(pointcloud)   ### 降低轨迹依赖 镜像翻转，使其见过反向回去的点云(难以处理回环、旋转的情况)
            pointcloud = self.flipY(pointcloud)   ### 但镜像翻转或许需要改变真值
        # rand = random.uniform(0, 1)
        # if rand < self.parmas.p_flipy:
        #     pointcloud = self.flipY(pointcloud)   ### 降低轨迹依赖   
