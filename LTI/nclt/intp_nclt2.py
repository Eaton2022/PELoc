...
if __name__ == '__main__':
    ...
    expanded_h5_path = osp.join(
        r"D:\文件\code_learn\3D-Detection-Tracking-Viewer-master\3D-Detection-Tracking-Viewer-master\h5\nclt",
        "velodyne_left_False_init_t=0.5_bias=+2_3.4.h5"
    )
    ...
    # 检查原始数据的形状
    print(f"Original timestamps shape: {ts.shape}")        
    print(f"Original poses shape: {ps.shape}")             


    point_cloud_pass = osp.join(r"E:\数据集\NCLT\2012-02-18\SPVNAS_velodyne_left_plane_segmented")  ## .bin
    point_cloud_pass_3 = osp.join(r"E:\数据集\NCLT\2012-02-18-2+\SPVNAS_velodyne_left_plane_segmented+2_test")  #### 3.4号增加旋转的影响
    ...
    ps_expanded = np.zeros((26797,12))
    for i in tqdm(range(26797), desc="Saving intp pose", unit="file"):
        R1 = ps[i,[0,1,2,4,5,6,8,9,10]].reshape(3,3)
        R2 = ps[i+1,[0,1,2,4,5,6,8,9,10]].reshape(3,3)
        t1 = ps[i,[3,7,11]]
        t2 = ps[i+1,[3,7,11]]
        pc = np.fromfile(osp.join(r"E:\数据集\NCLT\2012-02-18\SPVNAS_velodyne_left_plane_segmented", file_pc[i]),dtype = np.float32).reshape(-1,5)   ### (N,5)
        pc_cls = pc[:,3:5].reshape(-1,2)  #### (N,5) NCLT
        pc_c = pc[:,:3]
        ...
        for t in t_values:
            ...
            # 引入噪声和非线性扰动（同时作用于轨迹 t_in 和 transformed_points）
            random_offset = np.random.uniform(-0.3, 0.3, size=t_in.shape[0])  # 生成随机偏移量
            sinusoidal_offset = 0.1 * np.sin(t * np.pi * 2)  # 生成正弦扰动
            # 轨迹扰动
            t_in[:, 1] -= 2 + random_offset  
            t_in[:, 0] -= sinusoidal_offset  
            new_pose = np.concatenate((R_in, t_in.reshape(3,1)),1).reshape(12,)
            ps_expanded[i] = new_pose #### 生成新的位姿
            transformed_points = apply_relative_transform(pc_c, R_rel, T_rel)
            # 点云同步扰动
            transformed_points[:, 1] -= 2 + random_offset  # 保持与轨迹一致
            transformed_points[:, 0] -= sinusoidal_offset  # 同步正弦扰动
            transformed_points = np.concatenate((transformed_points,pc_cls),axis = 1).astype(np.float32)
            save_path_bin = osp.join(point_cloud_pass_3, f"{name_pc}_t=0.5_bais=+2.bin")   ### 生成.bin
            transformed_points.tofile(save_path_bin)
    print("done")    
    np.save("ps_0.5_bia=+2.npy",ps_expanded)
    
    
    ps_expanded = ps_expanded[:26797]  ### change
            
    num_repeat = 1
    ts_expanded = np.array([f"{t}_t=0.5_bais=+2" for t in ts], dtype="S")   ####
    ts_expanded = ts_expanded[:26797]
