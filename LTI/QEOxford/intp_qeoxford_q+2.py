...
if __name__ == '__main__':
    ...
    expanded_h5_path = osp.join(
        r"D:\文件\code_learn\3D-Detection-Tracking-Viewer-master\3D-Detection-Tracking-Viewer-master\h5",
        "velodyne_left_calibrateFalse_init_t=0.5_bias=+2_3.4_q.h5"
    )
    ...
    # point_cloud = np.fromfile(r"D:\文件\code_learn\3D-Detection-Tracking-Viewer-master\3D-Detection-Tracking-Viewer-master\pc_intp\try\1547216908207455.bin",dtype=np.float32).reshape(4,-1).transpose()[:,:3]
    point_cloud_pass = osp.join(r"D:\文件\语义点云-Oxford\first_pass\SPVNAS_velodyne_left_plane_segmented")
    point_cloud_pass_2 = osp.join(r"D:\文件\语义点云-Oxford\first_pass\SPVNAS_velodyne_left_plane_segmented_npy")
    point_cloud_pass_3 = osp.join(r"D:\文件\语义点云-Oxford\first_pass\SPVNAS_velodyne_left_plane_segmented_npy_0.5_bias=+2_3.4_q")  #### 3.4号增加旋转的影响
    ...
    ps_expanded = np.zeros((37665,12))
    for i in tqdm(range(37665), desc="Saving intp pose", unit="file"):   #### 有可能都要改
        R1 = ps[i][:9].reshape(3,3)
        R2 = ps[i+1][:9].reshape(3,3)
        t1 = ps[i][9:12]
        t2 = ps[i+1][9:12]

        pc = np.fromfile(osp.join("D:/文件\语义点云-Oxford/first_pass/SPVNAS_velodyne_left_plane_segmented/", file_pc[i]),dtype = np.float32).reshape(-1,4)
        ...
        for t in t_values:
            ...
            new_pose = np.concatenate((R_in, t_in),1)
